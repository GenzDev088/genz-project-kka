...
class BetterPlayerWrapper implements VideoController {
  ...
  void setQuality(QualityStream qs) async {
    ...
    if (track == null) {
      return print(
        "[ERROR] No matching track found for '${qs.resolution}', cannot set quality.",
      );
    }
  ...
  BetterPlayerAsmsTrack? _qsToAsmsTrack(QualityStream qs) {
    try {
      return controller.betterPlayerAsmsTracks.firstWhere(
        (element) {
          final resMatch =
              element.height ==
              (int.tryParse(qs.resolution.split("x").last) ?? 0);
          final bwMatch = element.bitrate == (qs.bandwidth ?? 0);
          return resMatch && bwMatch;
        },
        orElse: () {
          print(
            "[ERROR] No matching track found for '${qs.resolution}', defaulting to first track.",
          );
