...
class InfoProvider extends ChangeNotifier {
  ...
  set selectedSource(ProviderDetails val) {
    ...
    if (aspProvider != val.identifier) {
      saveAnimeSpecificPreference(
        id.toString(),
        AnimeSpecificPreference(preferredProvider: val.identifier),
      );
    }
  ...
  set preferDubs(bool val) {
    _preferDubs = val;
    UserPreferences.saveUserPreferences(UserPreferencesModal(preferDubs: val));
    paginate(epLinks);
    notifyListeners();
  }
  ...
  void init() async {
    ...
    final matchedSource = sources
        .where(
          (e) =>
              e.identifier ==
              (asp?.preferredProvider ??
                  currentUserSettings?.preferredProvider),
        )
  ...
  Future<void> loadPreferences() async {
    ...
    _viewMode = UserPreferencesModal.getViewModeIndex(
      preferences.episodesViewMode ?? EpisodeViewModes.tile,
    );
  ...
  Future<void> getWatched({bool refreshLastWatchDuration = false}) async {
    try {
      ...
      final supposedPageIndex =
          watched ~/ 25; //index increases when the episodes are >24

      _currentPageIndex = supposedPageIndex >= visibleEpList.length
          ? visibleEpList.length - 1
          : supposedPageIndex;

      if (refreshLastWatchDuration) {
        _lastWatchedDurationMap = (await getAnimeSpecificPreference(
          id.toString(),
        ))?.lastWatchDuration;
      }
    } catch (err) {
  ...
  Future<void> _getInfo(int id) async {
    ...
    try {
      if (currentUserSettings?.database == Databases.anilist &&
        ...
        simklFuture = () async {
          try {
            final res = await DatabaseHandler(
              database: Databases.simkl,
            ).search("https://anilist.co/anime/$id");
            if (res.isEmpty) return <AlternateDatabaseId>[];
            final info = await DatabaseHandler(
              database: Databases.simkl,
      ...
      final futures = await Future.wait([
        DatabaseHandler().getAnimeInfo(id),
        if (simklFuture != null) simklFuture,
      ]);
      ...
      s.addAll(
        futures.length > 1
            ? futures[1] as List<AlternateDatabaseId>
            : <AlternateDatabaseId>[],
      );
  ...
  void paginate(List<EpisodeDetails> links) {
    ...
    int?
    watchedProgressIndex; // js a variable to track the last watched episode in filtered list
    ...
    if (_visibleEpList.isEmpty) {
      ...
      _currentPageIndex = watchedProgressIndex != null
          ? (watchedProgressIndex ~/ 24)
          : 0;
    }
  ...
  Future<void> _search(String query) async {
    final sr = await sourceManager.searchInSource(
      selectedSource.identifier,
      query,
    );

    List<Map<String, String?>> match = sr
        .where((e) => e['name'] == query)
        .toList();
    if (match.isEmpty) match = sr;
    final links = await sourceManager.getAnimeEpisodes(
      selectedSource.identifier,
      match[0]['alias']!,
    );
    paginate(links);
  ...
  void clearLastWatchDuration() async {
    ...
    await saveAnimeSpecificPreference(
      id.toString(),
      AnimeSpecificPreference(lastWatchDuration: _lastWatchedDurationMap),
    );
    notifyListeners();
  }
