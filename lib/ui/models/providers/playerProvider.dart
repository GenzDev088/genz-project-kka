...
class PlayerProvider extends ChangeNotifier {
  ...
  List<double> get playbackSpeeds => [
    ...
    1.75,
    2,
    if ((currentUserSettings?.enableSuperSpeeds ?? false) &&
        !Platform.isWindows) ...[
      4,
      5,
      8,
  ...
  Future<void> playVideo(
    ...
    await _controller.initiateVideo(
      url,
      headers: currentStream.customHeaders,
      offline: offline,
    );

    if (seekTime != null)
      await _controller.seekTo(Duration(milliseconds: seekTime));
  }
  ...
  Future<void> fastForward(int seekDuration) async {
    ...
    } else if (duration > 0 && currentPosition + seekDuration >= duration) {
      await _controller.seekTo(
        Duration(milliseconds: _controller.duration! - 500),
      );
    } else {
      await _controller.seekTo(
        Duration(seconds: currentPosition + seekDuration),
      );
    }
  }
  ...
  void toggleControlsVisibility({bool? action = null}) {
    _state = _state.copyWith(
      controlsVisible: action ?? !_state.controlsVisible,
    );
    notifyListeners();
  }
  ...
  Future<void> setPip(bool val) async {
    ...
    if (Platform.isWindows) {
      ...
      await controller.setPip(
        true,
      ); // doesnt really matter for android since disabling is done by the system
    }
    notifyListeners();
  }
  ...
  void playPreloadedEpisode(PlayerDataProvider dataProvider) async {
    ...
    if (dataProvider.state.currentEpIndex + 1 >=
        dataProvider.epLinks.length /* add autonext */ ) {
      return;
    }

    if (dataProvider.state.preloadedSources.isNotEmpty) {

      List<VideoStream> preferredServerLink = dataProvider
          .state
          .preloadedSources
          .where(
            (source) =>
                source.server == dataProvider.state.currentStream.server,
          )
          .toList();
      ...
      final src = preferredServerLink.isNotEmpty
          ? (preferredServerLink.firstWhereOrNull(
                  (e) =>
                      e.quality.replaceAll(sizeRegex, "").trim() ==
                      dataProvider.state.currentStream.quality
  ...
  void _disablePip() {
    ...
    Provider.of<AppProvider>(
      AppWrapper.navKey.currentContext!,
      listen: false,
    ).showTitleBar = false;
  }
