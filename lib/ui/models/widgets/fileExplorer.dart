...
class _FileExplorerState extends State<FileExplorer> {
  ...
  final _rootDir =
      (currentUserSettings?.downloadPath ??
      "/storage/emulated/0/Download/animestream");
  ...
  Future<void> _readDir() async {
    currentDirPathSplit = currentDir.path.split(
      Platform.isWindows ? "\\" : "/",
    );
    setState(() {
      _loadingFiles = true;
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    currentDirPathSplit
                            .sublist(0, currentDirPathSplit.length - 1)
                            .join('/') +
                        "/",
          ...
          if (_loadingFiles)
            SizedBox(
              height: 20,
              width: 20,
              child: CircularProgressIndicator(color: appTheme.textSubColor),
            )
          else
            Text(
      ...
      child: Row(
        children: [
          Padding(
            ...
            child: Icon(Icons.folder_rounded, size: 28),
          ),
          Expanded(
            child: Text(
  ...
  Container _fileTile(FileSystemEntity entity) {
    ...
    final isSubtitleFile = entity.path.contains(
      RegExp(r'\.(ass|srt|vtt|txt)$', caseSensitive: false),
    );
    return _tappable(
      entity: entity,
      onTap: () {
        ...
        children: [
          Padding(
            padding: const EdgeInsets.only(
              right: 16,
            ), //12 means symmetrical to folder structure, but feels off here
            child: Icon(_getTypeIcon(entity.path.split(".").last), size: 28),
          ),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
                  ep != null
                      ? "Episode $ep ${isSubtitleFile ? "subtitles" : ""}"
                      : _getFileName(entity.path),
                  style: _titleStyle(),
                  overflow: TextOverflow.ellipsis,
                ),
    ...
    return showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Padding(
          ...
          child: Text(
            'Are you sure to delete "${_getFileName(entity.path)}" from your device?',
          ),
        ),
      ),
    );
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => MultiProvider(
          providers: [
            ChangeNotifierProvider(
              create: (context) => PlayerDataProvider(
                ...
                startIndex:
                    0, // does matter! [change with episode number, need to fw download methods]
          ...
          child: Watch(controller: controller, localSource: true),
        ),
      ),
    );
  }

  String _toMegs(int sizeInBytes) =>
