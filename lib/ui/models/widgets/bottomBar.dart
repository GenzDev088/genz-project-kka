...
class _AnimeStreamBottomBarState extends State<AnimeStreamBottomBar> {
  ...
  void initState() {
    super.initState();
    widget.controller.currentIndexNotifier.addListener(
      _onControllerIndexChanged,
    );
  }

  @override
  void dispose() {
    widget.controller.currentIndexNotifier.removeListener(
      _onControllerIndexChanged,
    );
    super.dispose();
  }
              ...
              child: Column(
                ...
                children: [
                  AnimatedSwitcher(
                    ...
                    duration: Duration(
                      milliseconds: widget.controller.animDuration,
                    ),
                    child: isSelected
                        ? Text(
                            e.title,
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(
        bottom: MediaQuery.paddingOf(context).bottom + widget.bottomPadding,
      ),
      child: Align(
        alignment: Alignment.bottomCenter,
...
class _BottomBarViewState extends State<BottomBarView> {
  ...
  Widget build(BuildContext context) {
    ...
    if ((widget.controller.length - widget.controller.nonViewIndices.length) !=
        widget.children.length) {
      throw Exception("Index out of bounds");
    }


    List<int> viewIndices = List.generate(
      widget.controller.length,
      (i) => i,
    ).where((i) => !widget.controller.nonViewIndices.contains(i)).toList();
