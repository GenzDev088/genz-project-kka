...
Future<BetterPlayerDataSource> dataSourceConfig(
  String url, {
...
Future<BetterPlayerVideoFormat> _getFormat(
  String url,
  ...
  if (mime == null || mime.contains("octet-stream")) {
    final urlPath = Uri.parse(
      url,
    ).path.toLowerCase(); //wrong variable name btw!
...
class PlayerLoadingWidget extends StatelessWidget {
  const PlayerLoadingWidget({super.key});
