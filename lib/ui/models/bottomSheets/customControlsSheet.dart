...
class CustomControls_BottomSheetState extends State<CustomControlsBottomSheet> {
  ...
  Future getSources() async {
    ...
    if (index == dp.state.currentEpIndex) {
      ...
      await SourceManager.instance.getStreams(
        dp.selectedSource,
        dp.epLinks[index].episodeLink,
        dub: dp.preferDubs,
        metadata: dp.epLinks[index].metadata,
        (list, finished) {
          if (list.length > 0) {
            if (mounted)
              setState(() {
  ...
  void play() async {
    await dp.extractCurrentStreamQualities().then((val) async {
      ...
      await pp.playVideo(
        dp.state.currentStream.url,
        currentStream: dp.state.currentStream,
        preserveProgress: index == dp.state.currentEpIndex,
      );
      await pp.setQuality(q);

      dp.getSkipTimesForCurrentEpisode(
        videoDuration: (pp.controller.duration ?? 0).toDouble(),
      );
