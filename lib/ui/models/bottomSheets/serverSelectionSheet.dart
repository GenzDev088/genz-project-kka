...
class ServerSelectionBottomSheet extends StatefulWidget {
  ...
  State<ServerSelectionBottomSheet> createState() =>
      ServerSelectionBottomSheetState();
}

class ServerSelectionBottomSheetState
        ...
        await src.getDownloadSources(
          ...
          (list, finished) {
            if (mounted)
              setState(() {
                if (finished) {
                  _isLoading = widget.type == ServerSheetType.download
                      ? true
                      : false;
                }
                ...
                if (widget.type == ServerSheetType.download) {
                  list.forEach((element) async {
                    qualities.add({
                      'url': element.url,
                      'server':
                          "${element.server}  ${element.backup ? "- backup" : ""}",
  ...
  Future<void> getQualities(VideoStream source) async {
    ...
    final ParsedHlsMaster list =
        await parseMasterPlaylist(
          source.url,
          customHeader: source.customHeaders,
        ).catchError((e) {
    ...
    return widget.type == ServerSheetType.watch
        ? ListView.builder(
            padding: EdgeInsets.symmetric(
              vertical: 10,
              horizontal: Platform.isAndroid ? 10 : 15,
            ),
            shrinkWrap: true,
            ...
            itemBuilder: (context, index) {
              ...
              return SourceTile(
                ...
                onTap: () async {
                  ...
                  final navigatorState = (Platform.isWindows
                      ? AppWrapper.navKey.currentState
                            ...
                            providers: [
                              ChangeNotifierProvider(
                                create: (context) => PlayerDataProvider(
                                  ...
                                  lastWatchDuration:
                                      provider.lastWatchedDurationMap?[provider
                                                  .watched <
                              ...
                              ChangeNotifierProvider(
                                create: (context) =>
                                    PlayerProvider(controller, true),
                              ),
                            ],
        ...
        : ListView.builder(
            ...
            shrinkWrap: true,
            itemBuilder: (BuildContext context, ind) => _DownloadTile(
              quality: qualities[ind],
              widget: widget,
              title: title,
            ),
          );
