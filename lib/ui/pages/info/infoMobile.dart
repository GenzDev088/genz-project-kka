...
class _InfoMobileState extends State<InfoMobile> {
  ...
  String getTitle(Map<String, String?> titles) {
    ...
    final customizedTitle = useNativeTitle
        ? titles['native'] ?? normalTitle
        : normalTitle;
      ...
      body: provider.infoLoadError
          ? Center(
              child: Column(
                ...
                children: [
                  Image.asset('lib/assets/images/broken_heart.png', scale: 7.5),
                  Padding(
                        ...
                        children: [
                          Container(
                            ...
                            child: ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                backgroundColor:
                                    _watchInfoButtonFocusNode.hasFocus
                          ...
                          if (provider.loggedIn)
                            Container(
                              child: ElevatedButton(
                                ...
                                child: Icon(
                                  InfoProvider.getTrackerIcon(
                                    provider.mediaListStatus,
                                  ),
                                  color: appTheme.accentColor,
                                  size: 28,
                                ),
  ...
  Column _watchItems(BuildContext context) {
    return Column(
      children: [
        ...
        if (provider.foundName != null && provider.epLinks.length > 0)
          _continueButton(),
        Container(
                    ...
                    if (provider.foundName != null)
                      Row(
                        ...
                        children: [
                          InkWell(
                            onTap: () =>
                                provider.preferDubs = !provider.preferDubs,
                            child: Container(
      ...
      child: InkWell(
        onTap: () {
          showModalBottomSheet(
            ...
            final links = await SourceManager.instance.getAnimeEpisodes(
              provider.selectedSource.identifier,
              result['alias'],
            );
            if (mounted)
              setState(() {
        ...
        child: Text(
          ...
          style: TextStyle(
            ...
            fontWeight: FontWeight.bold,
            shadows: [
              Shadow(color: appTheme.textMainColor, offset: Offset(0, -2)),
            ],
          ),
        ),
      ),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        image: DecorationImage(
          image: provider.data.banner != null
              ? NetworkImage(provider.data.banner!)
              : NetworkImage(provider.data.cover),
          fit: BoxFit.cover,
          opacity: 0.46,
        ),
      ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (provider.lastWatchedDurationMap?[provider.watched <
                          provider.epLinks.length
                      ? provider.watched + 1
                      : provider.watched] !=
                  null)
                Container(
                  width:
                      ...
                      (() {
                        final raw =
                            provider.lastWatchedDurationMap?[provider.watched <
                                    provider.epLinks.length
                                ? provider.watched + 1
                                : provider.watched] ??
                            0;
  ...
  Container _searchStatus() {
    String text =
        "searching: ${provider.data.title['english'] ?? provider.data.title['romaji'] ?? ''}";
    if (provider.foundName != null) {
  ...
  GridView _episodeTiles() {
    return GridView.builder(
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount:
            MediaQuery.of(context).orientation == Orientation.portrait ? 5 : 10,
      ),
      shrinkWrap: true,
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () async {
                              ...
                              if (!provider.selectedSource.supportDownloads &&
                                  ServerSheetType.values[ind] ==
                                      ServerSheetType.download) {
                                floatingSnackBar(
                                  "This source doesn't support downloading yet!",
                                );
                                return;
                              }
                              showModalBottomSheet(
                                ...
                                context: context,
                                backgroundColor:
                                    appTheme.modalSheetBackgroundColor,
                                builder: (BuildContext context) {
                                  return ServerSelectionBottomSheet(
                                    provider: provider,
                                    episodeIndex:
                                        provider.visibleEpList[provider
                                            .currentPageIndex][index]['realIndex'],
              ...
              children: [
                Opacity(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      image: DecorationImage(
                        image: NetworkImage(provider.data.cover),
                        fit: BoxFit.cover,
                      ),
                    ),
                    child: Column(
  ...
  GridView _episodes() {
    return GridView.builder(
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount:
            MediaQuery.orientationOf(context) == Orientation.portrait ? 1 : 2,
                      ...
                      children: [
                        Opacity(
                          ...
                          child: ShaderMask(
                            ...
                            child: Image.network(
                              provider
                                      .visibleEpList[provider
                                          .currentPageIndex][index]['epLink']
                                      .thumbnail ??
                                  provider.data.cover,
                              fit: BoxFit.cover,
                              width: 165,
            ...
            child: Column(
              children: [
                ...
                _buildInfoItems(
                  _infoLeft('Status'),
                  _infoRight(
                    '${provider.data.status ?? '??'}'.toLowerCase().replaceAll(
                      "_",
                      ' ',
                    ),
                  ),
                ...
                _buildInfoItems(
                  _infoLeft("Start Date"),
                  _infoRight("${provider.data.aired['start']}"),
                ),
                _buildInfoItems(
                  _infoLeft('Studios'),
                  _infoRight(
                    provider.data.studios.isEmpty
                        ? '??'
  ...
  SizedBox _buildRecAndRel(
    List<DatabaseRelatedRecommendation> data,
    bool recommended,
            ...
            onTap: () {
              ...
              if (!recommended)
                Navigator.of(context).push(
                  MaterialPageRoute(builder: (context) => Info(id: item.id)),
                );
            },
            child: Container(
              width: 130,
                          ...
                          child: ElevatedButton(
                            onPressed: () async {
                              try {
                                ...
                                floatingSnackBar(
                                  "Succesfully saved to your downloads folder!",
                                );
