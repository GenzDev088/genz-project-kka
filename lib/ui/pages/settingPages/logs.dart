...
class _LogScreenState extends State<LogScreen> {
  final loggers = [Logs.app, Logs.downloader, Logs.player];
    ...
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
          ...
          icon: Icon(Icons.arrow_back, color: appTheme.textMainColor),
        ),
        title: Text(
          "Logs",
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(Icons.save, color: appTheme.textMainColor),
            onPressed: () async {
              final downloadPath = await DownloaderHelper().getDownloadsPath();
              final dir = Directory(
                downloadPath + "/logs/${Logs.app.session}/",
              );
              await dir.create(recursive: true);
              for (final l in loggers) {
                l.log("Saving ${l.tag} log");
                await File(dir.path + "/${l.tag}.txt").writeAsString(
                  l.logNotifier.value.join("\n"),
        ...
        child: Column(
          children: [
            SegmentedButton(
              ...
              segments: [
                for (final l in loggers)
                  ButtonSegment(value: l.tag, label: Text("${l.tag}")),
              ],
            ...
            Expanded(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  border: Border.all(color: appTheme.textSubColor),
  ...
  TextStyle _logsTextStyle() {
    return TextStyle(fontFamily: "NunitoSans", fontSize: 14);
  }
}
