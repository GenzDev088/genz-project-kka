...
class _ThemeSettingState extends State<ThemeSetting> {
  ...
  void getAPILevel() {
    if (!kIsWeb && Platform.isAndroid)
      DeviceInfoPlugin().androidInfo.then(
        (val) => isAboveAndroid12 = (val.version.sdkInt >= 31),
      );
    else
  ...
  Future<void> applyTheme(int id) async {
    await setTheme(id);
    final theme = availableThemes
        .where((themeItem) => themeItem.id == id)
        .toList()[0];
    Provider.of<AppProvider>(
      context,
      listen: false,
    ).applyTheme(darkMode ? theme.theme : theme.lightVariant);
  }
                        ...
                        children: [
                            ...
                            onTapFunction: () async {
                              ...
                              if (!isAboveAndroid12)
                                return floatingSnackBar(
                                  "Android 12 or greater is required",
                                );
                              materialTheme = !materialTheme;
                              await Settings().writeSettings(
                                SettingsModal(materialTheme: materialTheme),
                              );
                              setState(() {});
                              if (materialTheme) {
                              ...
                              final t = availableThemes
                                  .where(
                                    (themeItem) =>
                                        themeItem.id == currentThemeId,
                                  )
                                  .toList()[0];
                              Provider.of<AppProvider>(
                                context,
                                ...
                                SegmentedButton(
                                  ...
                                  style: SegmentedButton.styleFrom(
                                    selectedBackgroundColor:
                                        appTheme.accentColor,
                                    selectedForegroundColor: appTheme
                                        .onAccent, //not workin for some reason
                          ...
                          ToggleItem(
                            onTapFunction: () async {
                              ...
                              setState(() {});
                              await Settings().writeSettings(
                                SettingsModal(nativeTitle: nativeTitle),
                              );
                              Provider.of<AppProvider>(
                                context,
                                listen: false,
                          ...
                          ToggleItem(
                            ...
                            onTapFunction: () async {
                              final thm = availableThemes.firstWhere(
                                (i) => i.id == currentThemeId,
                              );
                              ...
                              appTheme = darkMode
                                  ? AnimeStreamTheme(
                                      ...
                                      backgroundColor: AMOLEDBackgroundEnabled
                                          ? Colors.black
                                          : thm.theme.backgroundColor,
                                      backgroundSubColor:
                                          thm.theme.backgroundSubColor,
                                      textMainColor: thm.theme.textMainColor,
                                      textSubColor: thm.theme.textSubColor,
                                      modalSheetBackgroundColor:
                                          thm.theme.modalSheetBackgroundColor,
