...
class Anilist extends Database {
  ...
  Future<List<CurrentlyAiringResult>> getCurrentlyAiringAnime() async {
    ...
    final List<dynamic> data = await fetchQuery(
      query,
      RequestType.media,
  ...
  Future<List<RecentlyUpdatedResult>> recentlyUpdated() async {
    ...
    for (final recentlyUpdatedAnime in recentlyUpdatedAnimes) {
      if (recentlyUpdatedAnime['media']['isAdult'] == true ||
          recentlyUpdatedAnime['media']['countryOfOrigin'] != "JP")
        continue;
  ...
  Future<dynamic> fetchQuery(
    ...
    String _collectMessages(List<GraphQLError> errors) =>
    ...
    int? _extractStatusCode() {
      ...
      if (linkEx is ServerException) {
        ...
        final parsedCtxCode = parsed?.context
    ...
    if (res.hasException) {
      ...
      if (err.linkException != null) {
          ...
          if (parsed != null) {
            ...
            if (parsed.errors != null && parsed.errors!.isNotEmpty) {
              throw AnilistApiException(
                "GraphQL error: ${_collectMessages(parsed.errors!)}",
        ...
        throw AnilistApiException(
          "Network error: ${err.linkException.toString()}",
          statusCode: statusCode,
        );
      }
      ...
      if (err.graphqlErrors.isNotEmpty) {
        throw AnilistApiException(
          "GraphQL error: ${_collectMessages(err.graphqlErrors)}",
          statusCode: statusCode,
        );
      }
