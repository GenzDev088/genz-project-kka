...
abstract class BaseDownloader {
    ...
    task.sendPort?.send(
      DownloadMessage(
        ...
        extras: [
          resumeFrom,
          filepath,
        ], // Just pass which segment index it should resume from!
      ),
    );
  }
  ...
  void setCancelledStatus() {
    ...
    task.sendPort?.send(
      DownloadMessage(
        status: 'cancel',
        id: task.id,
        message: "Download cancelled on user request",
      ),
    );
  }
  ...
  void setFailedStatus(String message) {
    ...
    task.sendPort?.send(
      DownloadMessage(status: 'fail', id: task.id, message: message),
    );
  }
  ...
  Future<void> setUpPorts(DownloadTaskIsolate task) async {
    ...
    task.sendPort?.send(
      DownloadMessage(status: 'port', id: task.id, extras: [rp.sendPort]),
    );
    task.sendPort?.send(
      DownloadMessage(status: 'downloading', id: task.id),
    ); // set as downloading!
      ...
      if (msg is String) {
        switch (msg) {
          case 'cancel':
            {
              if (_status == DownloadStatus.paused) {
                ...
                Isolate.exit(
                  task.sendPort,
                  DownloadMessage(status: "cancel", id: task.id),
                );
              }
              _status = DownloadStatus.cancelled;
              break;
          ...
          default:
            {
              print(
                "Recieved undefined command $msg for isolate ${task.id}. Ignoring...",
              );
              return;
            }
        }
