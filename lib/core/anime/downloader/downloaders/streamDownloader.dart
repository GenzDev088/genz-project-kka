...
class StreamDownloader extends BaseDownloader {
  ...
  Future<void> download() async {
    ...
    if (task.subsUrl != null)
      downloadSubs(task.subsUrl!, task.fileName, finalPath);

    final output = File(finalPath);


    final out = await output.openWrite(
      mode: task.resumeFrom == 0 ? FileMode.write : FileMode.append,
    );

    try {
      final streamBaseLink = helper.makeBaseLink(task.url);
      final List<String> segments = await helper.getSegments(
        task.url,
      ...
      for (
        int i = task.resumeFrom.toInt();
        i < segmentsFiltered.length;
        ...
        if (status == DownloadStatus.cancelled) {
          ...
          try {
            timer = Timer(
              Duration(minutes: 1),
              () => completer!.completeError(Exception("Isolate wait timeout")),
            );
            await completer!.future;
            completer = null;
            timer?.cancel();
            timer =
                null; // Kill the timer if resumed within the 1 minute window
          } catch (err) {
        ...
        final batchEnd =
            (i + parallelDownloadsBatchSize < segmentsFiltered.length)
            ? i + parallelDownloadsBatchSize
        ...
        print(
          "[DOWNLOADER]<${task.id}> fetching segments [$i-$batchEnd of ${entries.length}]",
        );
        ...
        final futures = batch.map((entry) async {
          ...
          final res = await helper.downloadSegmentWithRetries(
            uri,
            task.retryAttempts,
            customHeaders: task.customHeaders,
          );
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            if (helper.encryptionKey != null) {
              buffers.add(
                BufferItem(
                  index: segmentNumber,
                  buffer: helper.decryptSegment(res.bodyBytes),
                ),
              );
            } else {
              buffers.add(
                BufferItem(index: segmentNumber, buffer: res.bodyBytes),
              );
            }
          } else {
            print(
              "Error for segment: $uri, body: ${res.body}, res head: ${res.headers}",
            );
            throw new Exception(
              "ERR_REQ_FAILED. Got Status: ${res.statusCode} for segment: $segmentNumber",
            );
          }
        });
      ...
      print(
        "[DOWNLOADER]<${task.id}> Closing download with state: ${status.name}",
      );
    } catch (err) {
      print(err);
