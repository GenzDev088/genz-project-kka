...
class VideoDownloader extends BaseDownloader {
  ...
  Future<void> download() async {
    ...
    extensionGuess =
        ['mp4', 'mkv', 'avi', 'webm', 'flv'].contains(extensionGuess)
        ? extensionGuess
        : null;
    ...
    if (task.resumeFrom != 0) {
      final doesServerSupportRangeHeader = await helper.checkRangeSupport(
        Uri.parse(task.url),
      );
        ...
        (chunk) async {
          ...
          if (status == DownloadStatus.cancelled) {
            ...
            try {
              timer = Timer(
                Duration(minutes: 1),
                () => super.completer?.completeError(Exception("Timeout!")),
              );
