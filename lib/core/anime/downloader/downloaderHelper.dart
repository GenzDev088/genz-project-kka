...
class DownloaderHelper {
  ...
  Future<String> makeDirectory({
    ...
    final animeName = fileName
        .replaceAll(RegExp(r'\s+ep\s*\d+\s*$', caseSensitive: false), '')
        .trim();
  ...
  Future<Response> downloadSegment(
    String url, {
    ...
    try {
      ...
      throw Exception(
        "Segment download failed with status code ${res.statusCode}",
      );
    } catch (err) {
  ...
  Uint8List decryptSegment(Uint8List buffer) {
    try {
      ...
      final decryptedBuffer = encrypt.decryptBytes(
        Encrypted(buffer),
        iv: IV.fromLength(16),
      );
    ...
    while (currentAttempt < totalAttempts) {
      try {
        ...
        final res = await downloadSegment(url, customHeaders: customHeaders)
            .timeout(
              Duration(seconds: 10),
              onTimeout: () => throw Exception("FAILED DOWNLOAD ATTEMPT"),
            );
        return res;
      } catch (err) {
        if (currentAttempt >= totalAttempts) {
          throw Exception(
            "NUMBER OF DOWNLOAD ATTEMPTS EXCEEDED, KILLING THE DOWNLOAD.",
          );
        }
      }
    }
        ...
        if (encryptionKey == null && line.startsWith("#EXT-X-KEY:METHOD=")) {
          ...
          if (match != null) {
            ...
            keyLink = keyLink.startsWith("http")
                ? keyLink
                : makeBaseLink(url) + "/$keyLink";
            encryptionKey = (await get(
              Uri.parse(keyLink),
  ...
  void sendCompletedNotif(int id, String fileName, String downloadPath) {
    _notifierService.downloadCompletionNotification(
      id: id,
      fileName: fileName,
      path: downloadPath,
    );
