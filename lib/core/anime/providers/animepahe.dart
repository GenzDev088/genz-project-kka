...
class AnimePahe extends AnimeProvider {
  ...
  Future<List<Map<String, dynamic>>> getAnimeEpisodeLink(
    ...
    List list = [];
    final String url =
        "https://animepahe.si/api?m=release&id=$session&sort=episode_asc`";
    ...
    for (int i = 1; i < totalPages; i++) {
      if (i > 5)
        break; //sorry long episoded anime fans, its just not worth 40 requests. the time it gon take is huge!
    ...
    for (int i = 0; i < list.length; i++) {
      final episodeLink =
          "https://animepahe.si/play/$session/${list[i]['session']}";
      final String? thumbnail = list[i]['snapshot'];
      final filler = list[i]['filler'] != 0;
      final String? title = ((list[i]['title'] as String?)?.isEmpty ?? true)
          ? null
          : list[i]['title'];
  ...
  Future<void> getStreams(
    ...
    return await getDownloadSources(
      episodeUrl,
      update,
      dub: dub,
      metadata: metadata,
      ...
      downloadLink
          .then((val) {
            ...
            update([
              VideoStream(
                quality:
                    (e['quality'] ?? "unknown-quality") + " [${e['size']}]",
                server: e['server'] ?? "unknown",
                url: val.$1,
                customHeaders: {'Referer': val.$2},
                backup: false,
                subtitle: null,
                subtitleFormat: null,
              ),
