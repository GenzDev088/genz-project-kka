...
class ProvidersPreferences {
  ...
  Future<ProviderDetails?> getProvider(String identifier) async {
    ...
    final Map<String, dynamic>? provider = (await box.get(identifier) as Map)
        .cast();
    await box.close();
  ...
  Future<List<ProviderDetails>> listAllProviders() async {
    ...
    final List<Map<String, dynamic>> mappedList = providers
        .map((it) => Map.from(it as Map).cast<String, dynamic>())
        .toList();
    await box.close();
