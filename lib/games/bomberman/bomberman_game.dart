...
class _BombermanGameState extends State<BombermanGame> {
  ...
  void initializeGame() {
    ...
    grid = List.generate(
      gridSize,
      (i) => List.generate(gridSize, (j) => TileType.empty),
    );
  ...
  void placeBomb() {
    ...
    bool bombExists = bombs.any(
      (bomb) => bomb.x == player.x && bomb.y == player.y,
    );
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  ElevatedButton(onPressed: placeBomb, child: const Text('💣')),
                ],
              ),
        ...
        .map(
          (enemy) => Positioned(
            ...
            child: Container(
              width: tileSize.toDouble(),
              height: tileSize.toDouble(),
              child: const Icon(Icons.bug_report, color: Colors.red, size: 25),
            ),
          ),
        )
        .toList();
