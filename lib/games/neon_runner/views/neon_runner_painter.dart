...
class NeonRunnerPainter extends CustomPainter {
  ...
  void _drawEnhancedBackground(Canvas canvas, Size size) {
    ...
    final radialGradient = RadialGradient(
      ...
      radius: 1.5,
      colors: [const Color(0xFF2D1B69).withOpacity(0.2), Colors.transparent],
    );
  ...
  void _drawEnhancedGrid(Canvas canvas, Size size) {
    ...
    for (double y = gameState.groundY; y <= size.height; y += gridSize * 0.8) {
      canvas.drawLine(Offset(0, y), Offset(size.width, y), gridPaint1);
    }
  }
  ...
  void _drawRockDetails(Canvas canvas, Obstacle obstacle) {
    ...
    canvas.drawCircle(Offset(centerX - 5, centerY - 5), 3, detailPaint);
    canvas.drawCircle(Offset(centerX + 3, centerY + 2), 2, detailPaint);
  }
    ...
    for (int i = 0; i < 6; i++) {
      ...
      canvas.drawLine(
        ...
        Paint()
          ..color = const Color(
            0xFF00FFFF,
          ).withOpacity((1 - animationTime) * 0.8)
          ..strokeWidth = 2
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 4),
      );
    }
  ...
  void _drawAnimatedGameOverIcon(Canvas canvas, Offset center, double size) {
    ...
    canvas.drawLine(
      Offset(
        animatedCenter.dx - size * 0.2 + eyeOffset,
        animatedCenter.dy - size * 0.1,
      ),
      Offset(
        animatedCenter.dx - size * 0.05 + eyeOffset,
