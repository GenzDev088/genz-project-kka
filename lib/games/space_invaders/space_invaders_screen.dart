...
class _SpaceInvadersScreenState extends State<SpaceInvadersScreen>
  ...
  static const Duration _autoShootInterval = Duration(
    milliseconds: 300,
  ...
  void _spawnEnemy() {
    ...
    setState(() {
      _gameState = _gameState.copyWith(enemies: [..._gameState.enemies, enemy]);
    });
  }
  ...
  void _handlePanUpdate(DragUpdateDetails details) {
    ...
    newPosition = Offset(
      math.max(
        20,
        math.min(_gameSize.width - 20, newPosition.dx + _panVelocity),
      ),
      newPosition.dy,
    );
  ...
  void _cleanupObjects() {
    ...
    final visibleBullets = _gameState.bullets
        .where((bullet) => bullet.position.dy > -10)
        .toList();
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Positioned.fill(child: _buildGameArea()),
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              _buildRetroStatItem(
                'Level',
                _gameState.level.toString(),
                const Color(0xFF00FFFF),
              ),
              _buildRetroStatItem(
                'Lives',
                _gameState.lives.toString(),
      ...
      child: GestureDetector(
        ...
        child: Stack(
          children: [
            ...
            Positioned.fill(
              child: CustomPaint(painter: SpaceInvadersPainter(_gameState)),
            ),
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      const Icon(Icons.replay, color: Colors.black, size: 20),
...
class Bullet {
  ...
  const Bullet({required this.position, required this.velocity});

  Bullet copyWith({Offset? position, Offset? velocity}) {
    return Bullet(
...
class SpaceInvadersPainter extends CustomPainter {
  ...
  void _drawPlayer(Canvas canvas, Offset position) {
    ...
    canvas.drawCircle(Offset(position.dx, position.dy + 10), 5, glowPaint);
  }
...
class GalaxyBackgroundPainter extends CustomPainter {
  ...
  void _drawNebulaClouds(Canvas canvas, Size size) {
    for (final cloud in nebulaClouds) {
      ...
      canvas.drawCircle(Offset(x, y), cloud.size * pulse, paint);
      ...
      canvas.drawCircle(Offset(x, y), cloud.size * 0.6 * pulse, innerPaint);
    }
  }
  ...
  void _drawCosmicDust(Canvas canvas, Size size) {
    for (final dust in cosmicDust) {
      ...
      canvas.drawCircle(Offset(x, y), dust.size, paint);
    }
  }
  ...
  void _drawPlanets(Canvas canvas, Size size) {
    for (final planet in planets) {
      ...
      canvas.drawCircle(Offset(x, y), planet.size * 1.2, atmospherePaint);
      ...
      canvas.drawCircle(Offset(x, y), planet.size, surfacePaint);
      ...
      if (planet.hasRings) {
        ...
        canvas.drawCircle(Offset(x, y), planet.size * 1.5, ringPaint);

        canvas.drawCircle(Offset(x, y), planet.size * 1.7, ringPaint);
      }
  ...
  void _drawAsteroids(Canvas canvas, Size size) {
    for (final asteroid in asteroids) {
      ...
      final y =
          (asteroid.position.dy * size.height +
