...
class _PacmanScreenState extends State<PacmanScreen>
  ...
  void initState() {
    ...
    _mouthAnimation =
        Tween<double>(begin: 0.0, end: 0.25).animate(
          ...
          return Container(
            ...
            child: Stack(
              children: [
                ...
                ..._controller.dots.map(
                  (p) => _buildDot(p, cellWidth, cellHeight),
                ),
                ..._controller.powerPellets.map(
                  (p) => _buildPowerPellet(p, cellWidth, cellHeight),
                ),
                _buildGhostDoor(cellWidth, cellHeight),
                ..._controller.ghosts.map(
                  (g) => _buildGhost(g, cellWidth, cellHeight),
                ),
                _buildPlayer(_controller.player, cellWidth, cellHeight),
                if (_controller.cherryPosition != null)
                  _buildCherry(
                    _controller.cherryPosition!,
                    cellWidth,
                    cellHeight,
                  ),
  ...
  Widget _buildGhostDoor(double cw, double ch) {
    return Positioned(
      top: 13 * ch - 1,
      left: 13 * cw,
      child: Container(width: 2 * cw, height: 2, color: Colors.pinkAccent),
    );
  }
...
class _MazePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color =
          const Color(0xFF2121DE) // Classic Pac-Man Blue
