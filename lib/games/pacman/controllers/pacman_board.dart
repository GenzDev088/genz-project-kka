...
class PacmanGameController extends ChangeNotifier {
  ...
  void _initialize() {
    ...
    ghosts = [
      Ghost(
        position: const Point(9, 8),
        imageAsset: 'assets/ghost.png',
        isReleased: true,
      ), // Blinky
      Ghost(
        position: const Point(8, 10),
        imageAsset: 'assets/ghost2.png',
      ), // Pinky
      Ghost(
        position: const Point(10, 10),
        imageAsset: 'assets/ghost3.png',
      ), // Inky
      Ghost(
        position: const Point(9, 9),
        imageAsset: 'assets/ghost.png',
      ), // Clyde
    ];
    powerPellets = {
      ...
      for (var x = 1; x < colCount - 1; x++)
        for (var y = 1; y < rowCount - 1; y++)
          if (!walls.contains(Point(x, y)) &&
              !powerPellets.contains(Point(x, y)))
            Point(x, y),
    };
    score = 0;
    isGameOver = false;
  ...
  void _moveGhost(Ghost ghost) {
    ...
    print(
      '--- $ghostName | Pos: (${ghost.position.x}, ${ghost.position.y}), Released: ${ghost.isReleased} ---',
    );
    ...
    if (isInPen) {
      ...
      if (ghost.position.y != centerPenY) {
        ...
        bestDirection = (ghost.position.x < doorPosition.x)
            ? Direction.right
            : Direction.left; // Then align with the door
      } else {
      ...
      switch (ghost.state) {
        case GhostState.normal:
          bestDirection = _getChaseDirection(
            ghost.position,
            player.position,
            ghost.direction,
          );
        ...
        case GhostState.frightened:
          bestDirection = _getFrightenedDirection(
            ghost.position,
            player.position,
            ghost.direction,
          );
    ...
    if (!walls.contains(nextGhostPos) || ghostDoor.contains(nextGhostPos)) {
      ...
      print(
        '$ghostName | Action: MOVED to (${nextGhostPos.x}, ${nextGhostPos.y})',
      );
    } else {
  ...
  Direction _getFrightenedDirection(
    Point from,
    Point to,
    Direction currentDirection,
  ) {
  ...
  Set<Point> _getClassicWalls() {
    return {
      ...
      for (var i = 7; i < 12; i++)
        if (i != 9) Point(i, 7), // Top wall with door opening
